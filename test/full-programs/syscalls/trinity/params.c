...
static void usage(void)
{
  ...
  outputerr(" -j#: target specific syscall (takes syscall number as parameter.\n");
...
static const char paramstr[] = "a:b:c:C:dDg:hIl:j:LN:mP:E:pqr:s:T:SV:vx:X";
...
void parse_args(int argc, char *argv[])
{
    ...
    case 'j':
			/* syscalls are all disabled at this point. enable the syscall we care about BY NUMBER. */
			do_specific_syscall = TRUE;
      int syscallno = atoi(optarg);
			toggle_syscall_n(syscallno, TRUE, optarg, "by number");
			break;
