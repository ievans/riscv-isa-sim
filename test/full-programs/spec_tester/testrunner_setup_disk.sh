...
dd if=/dev/zero of=root.bin bs=3M count=64 2> /dev/null
sudo mkfs.ext2 -F root.bin 2> /dev/null
...
if [ "$3" != "" ]; then
    ...
    echo "::sysinit:/riscv_tests/$(basename $(dirname $3))/$(basename $3) /riscv_tests/$(basename $(dirname $3))/$4" | cat etc/inittab - > /tmp/out && mv /tmp/out etc/inittab
